...
export default function CampaignDetail() {
  ...
  const copyTable = () => {
    ...
    // กำหนด index ของคอลัมน์ที่ต้องการคัดลอก
    const columnsToCopy = [0, 1, 2, 3, 4, 5, 6, 7];

    let textToCopy = "";
    const rows = table.querySelectorAll("tr");
    rows.forEach((row) => {
      // ดึง cell ทั้งหมดในแถว (th และ td)
      const cells = row.querySelectorAll("th, td");
      const rowData = [];

      // วน loop ตาม index ที่ระบุใน columnsToCopy
      columnsToCopy.forEach((colIndex) => {
        const cell = cells[colIndex];
        // ถ้ามี cell ในตำแหน่งนั้น ให้นำค่า innerText (ลบ newline) มาเก็บ
        const cellText = cell
      ...
      // คั่นคอลัมน์ด้วย tab และคั่นแต่ละแถวด้วย newline
  ...
  const exportToExcel = () => {
    ...
    // แปลงตาราง (table DOM) เป็น Workbook
