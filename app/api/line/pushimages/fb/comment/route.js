...
export async function POST(req) {
  try {
    ...
    if (existingRecord && existingRecord.url_img) {
      updatedFileNamesString =
        existingRecord.url_img + "," + updatedFileNamesString;
    }
    ...
    if (pushmessage && Number(pushmessage) > 0) {
      const pageAccessToken = process.env.NEXT_PUBLIC_FACEBOOK_PAGE_TOKEN;
    
      for (const imageUrl of imageUrls) {
        await fetch(`https://graph.facebook.com/v19.0/${userid}/comments`, {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            message: "🙏 ขออนุโมทนาบุญด้วยครับ",
            attachment_url: process.env.NEXT_PUBLIC_BASE_URL + imageUrl,
          }),
        });
      }
    }    
